...
// new sample (nominal) and 20_60
// constexpr std::array<double, 5> nomBDT = {0.09, 0.08, 0.03, 0.02, 0.07};
// constexpr std::array<double, 5> preBDT = {0.51, 0.45, 0.38, 0.30, 0.25};
constexpr std::array<double, 5> nomBDT = {0.08, 0.07, 0.0, 0.02, 0.04};
...
void BPPreSel(int doMC, bool nominal){
  ...
  TFile* flist = TFile::Open(inList);
  TTree* tlist = flist->Get<TTree>("Bfinder/root");
  int EvtNo;
  int flumi;
  ...
  if(doMC == 0) FileName = "~/dat/presel/output/BP_Data_BDTs_nom.root";
  // if(doMC == 1) FileName = "~/dat/presel/output/BP_MC_BDTs_nom.root";
  if(doMC == 1) FileName = "~/dat/presel/output/_BP_MC_BDTs_pre_tnp.root";
  if (jpsi) {
    // FileName = "~/dat/presel/output/jpsinp_nom.root";
    FileName = "~/dat/presel/output/jpsinp_pre.root";
  }
  ...
  if (nominal) {
    if(doMC == 0) FileName = "~/dat/presel/output/BP_Data_BDTs_nom.root";
    // if(doMC == 1) FileName = "~/dat/presel/output/BP_MC_BDTs_nom.root";
    // if(doMC == 1) FileName = "~/dat/presel/output/BP_MC_BDTs_pre_tnp.root";
    if(doMC == 1) FileName = "~/dat/presel/output/_BP_MC_BDTs_nom_tnp.root";
    if (jpsi) {
      // FileName = "~/dat/presel/output/jpsinp_nom.root";
      FileName = "~/dat/presel/output/jpsinp_nom.root";
    }
	...
	// TTree * BDT7 = (TTree *) fin->Get("BDT_pt_20_60");
  ...
  if (!jpsi) {
	...
	ntHi->SetBranchAddress("weight",&weight);
	   ...
	   // BDT7->SetBranchAddress("BDT_pt_20_60",BDT_pt_20_60);
  ...
  if(doMC == 0) 	foutstr = "~/dat/presel/BPData_presel.root";
  // if(doMC == 1) 	foutstr = "~/dat/presel/BPMC_presel.root";
  if(doMC == 1) 	foutstr = "~/dat/presel/new/BPMC_presel.root";
  if (filter) {
    if(doMC == 0) 	foutstr = "~/dat/presel/BPData_presel_sort.root";
    if(doMC == 1) 	foutstr = "~/dat/presel/BPMC_presel_sort.root";
  }
  if (jpsi) {
    foutstr = "~/dat/presel/jpsinp_presel.root";
  }
  if (nominal) {
    if(doMC == 0) 	foutstr = "~/dat/presel/BPData_nom.root";
    // if(doMC == 1) 	foutstr = "~/dat/presel/BPMC_nom.root";
    if(doMC == 1) 	foutstr = "~/dat/presel/new/BPMC_nom.root";
    if (jpsi) {
      foutstr = "~/dat/presel/jpsinp_nom.root";
    }
  }
  if (noBDT) {
	...
	Float_t BDT_pt_20_60_New[NCand];
	...
	ntKp->SetBranchAddress("Bmu2isTriggered",Bmu2isTriggered);
	   ...
	   // EffInfoTree->Branch("BDT_pt_20_60",BDT_pt_20_60_New,"BDT_pt_20_60/F");
...
//	EffInfoTree->Branch("HLT_HIL3Mu0NHitQ10_L2Mu0_MAXdR3p5_M1to5_v1",&HLT_HIL3Mu0NHitQ10_L2Mu0_MAXdR3p5_M1to5_v1New,"HLT_HIL3Mu0NHitQ10_L2Mu0_MAXdR3p5_M1to5_v1/I");
  ...
  if (!jpsi) {
    ...
    std::array<double, 5> bdtCut = nomBDT;

		for(int j =0; j < Bsize; j++){

      bool passBDT = ( (Bpt[j] > 5 && Bpt[j] < 7 && BDT_pt_5_7[j] > bdtCut[0]) ||
                       (Bpt[j] > 7 && Bpt[j] < 10 && BDT_pt_7_10[j] > bdtCut[1]) ||
                       (Bpt[j] > 10 && Bpt[j] < 15 && BDT_pt_10_15[j] > bdtCut[2]) ||
                       (Bpt[j] > 15 && Bpt[j] < 20 && BDT_pt_15_20[j] > bdtCut[3])  ||
      (Bpt[j] > 20 && Bpt[j] < 50 && BDT_pt_20_50[j] > bdtCut[4]) ||
      (Bpt[j] > 50 && Bpt[j] < 60));
      ...
      if ((pPAprimaryVertexFilter == 1 && pBeamScrapingFilter == 1 && HLT_HIL1DoubleMu0_v1 == 1)  &&  (Bmu1isTriggered[j] == 1 && Bmu2isTriggered[j] == 1 ) && (Btrk1Pt[j] > 0.5 && Bpt[j] > 2 && abs(Btrk1Eta[j]-0.0) < 2.4  && (TMath::Abs(By[j])<2.4&&((abs(Bmu1eta[j])<1.2&&Bmu1pt[j]>3.5)||(abs(Bmu1eta[j])>1.2&&abs(Bmu1eta[j])<2.1&&Bmu1pt[j]>(5.47-1.89*abs(Bmu1eta[j])))||(abs(Bmu1eta[j])>2.1&&abs(Bmu1eta[j])<2.4&&Bmu1pt[j]>1.5))&&((abs(Bmu2eta[j])<1.2&&Bmu2pt[j]>3.5)||(abs(Bmu2eta[j])>1.2&&abs(Bmu2eta[j])<2.1&&Bmu2pt[j]>(5.47-1.89*abs(Bmu2eta[j])))||(abs(Bmu2eta[j])>2.1&&abs(Bmu2eta[j])<2.4&&Bmu2pt[j]>1.5))&&Bmu1TMOneStationTight[j]&&Bmu2TMOneStationTight[j]&&Bmu1InPixelLayer[j]>0&&(Bmu1InPixelLayer[j]+Bmu1InStripLayer[j])>5&&Bmu2InPixelLayer[j]>0&&(Bmu2InPixelLayer[j]+Bmu2InStripLayer[j])>5&&Bmu1dxyPV[j]<0.3&&Bmu2dxyPV[j]<0.3&&Bmu1dzPV[j]<20&&Bmu2dzPV[j]<20&&Bmu1isTrackerMuon[j]&&Bmu2isTrackerMuon[j]&&Bmu1isGlobalMuon[j]&&Bmu2isGlobalMuon[j]&&Btrk1highPurity[j]&&abs(Btrk1Eta[j])<2.4&&Btrk1Pt[j]>0.5)  && (Btrk1PixelHit[j] + Btrk1StripHit[j] > 10) &&  (Btrk1PtErr[j]/Btrk1Pt[j] < ptErr[Tracking::loose]) && Btrk1Chi2ndf[j]/(Btrk1nStripLayer[j]+Btrk1nPixelLayer[j]) < chi2Nlayer[Tracking::loose]   && (abs(PVz)<15))){
        ...
        if (passTracking(Tracking::tight)) {
          ...
          trackSelection = Tracking::loose;
				...
				// BDT_pt_20_60_New[iPass] = BDT_pt_20_60[j];
