...
void Bfinder::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
	...
	*/
	...
	if( VertexHandle.isValid() && !VertexHandle.failedToGet() && VertexHandle->size() > 0) {
		...
		//	std::cout << "VtxInfo.Size = " << VtxInfo.Size << std::endl;
	...
	TrkEff2017pp trkEff =  TrkEff2017pp(false, "/afs/cern.ch/user/h/hmarques/CMSSW_9_4_10/src/Bfinder/Bfinder/src/");
	for(unsigned it=0; it<tracks->size(); ++it){
		...
		if(pt<=0.4) continue;
		nMult_ass_good += trkEff.getCorrection(pt,eta);
		//nMult_ass_good++;
	}
